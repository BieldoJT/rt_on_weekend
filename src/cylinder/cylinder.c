...
// Calcula as interseções com os planos das tampas superior e inferior
static void intersect_caps(t_cylinder *cyl, t_ray r, t_intersections *xs)
{
    ...
    if (fabs(denom) < EPSILON) // Raio paralelo às tampas, sem interseção
        return;
    // Verifica a tampa inferior
...
// Calcula as interseções com as paredes do cilindro
static void intersect_walls(t_cylinder *cyl, t_ray r, t_intersections *xs)
{
    ...
    abc[1] = 2 * (vec3_dot(r.dir, oc) - (vec3_dot(r.dir, cyl->axis) * vec3_dot(oc, cyl->axis)));
    abc[2] = vec3_length_squared(oc) - pow(vec3_dot(oc, cyl->axis), 2) - (cyl->radius * cyl->radius);
    if (fabs(abc[0]) < EPSILON) // Raio é paralelo à parede do cilindro
        return;
    if (!bhaskara(abc, &t[0], &t[1]))
        return;
    // Verifica se a primeira interseção (t0) está dentro dos limites de altura
    y = vec3_dot(vec3_sub(ray_at(r, t[0]), cyl->center), cyl->axis);
    if (y > -cyl->height / 2.0 && y < cyl->height / 2.0)
        add_intersection(xs, t[0]);
    // Verifica se a segunda interseção (t1) está dentro dos limites de altura
    y = vec3_dot(vec3_sub(ray_at(r, t[1]), cyl->center), cyl->axis);
    if (y > -cyl->height / 2.0 && y < cyl->height / 2.0)
        add_intersection(xs, t[1]);
}

// Calcula o vetor normal no ponto de interseção
static t_vec3 get_cylinder_normal_at(t_cylinder *cyl, t_vec3 p)
{
    ...
    // Ponto está na tampa superior
    if (dist >= half_height - EPSILON)
        return (cyl->axis);
    // Ponto está na tampa inferior
    if (dist <= -half_height + EPSILON)
        return (vec3_mul(cyl->axis, -1.0));
    // Ponto está na parede lateral
    else
    {
        t_vec3 projected = vec3_mul(cyl->axis, dist);
        return (vec3_normalize(vec3_sub(oc, projected)));
    }
}

// Função principal que orquestra a verificação de colisão com o cilindro
...
// Função para criar e inicializar um objeto cilindro
